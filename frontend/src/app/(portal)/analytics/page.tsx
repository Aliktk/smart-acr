  ...
  return (
    <div className="mx-auto max-w-screen-xl space-y-5 p-6">
      <div>
        ...
        <h1 className="mt-1 text-xl font-bold text-[#111827] dark:text-slate-100">Leadership Analytics</h1>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          <div key={item.label} className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-[var(--card)] px-4 py-4 shadow-sm">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#94A3B8]">{item.label}</p>
            <p className="mt-2 text-2xl font-semibold text-[#111827] dark:text-slate-100">{item.value}</p>
          </div>
        ))}
      </div>

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <section className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-[var(--card)] p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-[#111827] dark:text-slate-100">Wing-wise Trends</h2>
          <div className="mt-4 space-y-3">
            {data?.wingWiseTrends.map((item) => (
              <div key={item.name} className="rounded-lg border border-gray-100 bg-[#F9FAFB] p-4">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-[#111827] dark:text-slate-100">{item.name}</p>
        ...
        <section className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-[var(--card)] p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-[#111827] dark:text-slate-100">Backlog Distribution</h2>
          <div className="mt-4 space-y-3">
            {data?.backlogDistribution.map((item) => (
              <div key={item.name}>
                <div className="mb-1 flex items-center justify-between text-sm text-gray-600">
                  <span>{item.name}</span>
                  <span className="font-semibold text-[#111827] dark:text-slate-100">{item.pending}</span>
                </div>
