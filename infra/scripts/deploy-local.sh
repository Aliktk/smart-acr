...
# ─────────────────────────────────────────────────────────────────────────────
# deploy-local.sh
# One-step deployment for local Windows/Mac testing (port 8080, HTTP only).
# Run from the REPO ROOT: bash infra/scripts/deploy-local.sh
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
BACKEND_ENV="${ROOT_DIR}/infra/docker/env/backend.local.env"
FRONTEND_ENV="${ROOT_DIR}/infra/docker/env/frontend.local.env"
...
if [[ ! -f "${BACKEND_ENV}" ]]; then
  ...
  echo "       Run: cp infra/docker/config/backend.local.env.example infra/docker/env/backend.local.env"
...
if [[ ! -f "${FRONTEND_ENV}" ]]; then
  ...
  echo "       Run: cp infra/docker/config/frontend.local.env.example infra/docker/env/frontend.local.env"
  exit 1
fi

if grep -q "CHANGE_ME" "${BACKEND_ENV}"; then
  echo "ERROR: backend.local.env still contains CHANGE_ME placeholder values."
...
for i in $(seq 1 12); do
  if curl -fsS --max-time 5 "http://127.0.0.1:8080/api/v1/health/ready" >/dev/null 2>&1; then
    API_OK=true
    break
  fi
...
if [[ "${API_OK}" == "true" ]] && [[ "${WEB_OK}" == "true" ]]; then
  ...
  echo "  API health:      http://localhost:8080/api/v1/health/ready"
  echo "======================================================"
else
  ...
  echo ""
  echo "  Check logs with: docker compose -f infra/docker/docker-compose.local.yml logs -f"
  echo "======================================================"
  exit 1
fi
